...
$CacheBackup = Join-Path $CacheDir "plan_cache_backup_before_gemini_flash_$RunTimestamp.json"
$SummaryLog = Join-Path $LogDir "gemini_flash_all_$RunTimestamp.summary.log"
...
function Invoke-BenchmarkRun {
    ...
    Set-RunEnvironment -Method $Method -ExtraEnv $ExtraEnv
    Write-Summary "START $Name method=$Method main_model=$env:MAIN_MODEL"
...
Write-Summary "Gemini 2.5 Flash benchmark batch started"
Write-Summary "Existing cache backup: $CacheBackup"

Invoke-BenchmarkRun -Name "gemini_flash_baseline_none" -Method "none"
Invoke-BenchmarkRun -Name "gemini_flash_filtering" -Method "filtering"

"{}" | Set-Content -LiteralPath $CacheFile -Encoding UTF8
Write-Summary "Cache cleared for cold caching run: $CacheFile"
Invoke-BenchmarkRun -Name "gemini_flash_caching_cold" -Method "caching"
Copy-Item -LiteralPath $CacheFile -Destination (Join-Path $CacheDir "plan_cache_after_gemini_flash_cold_$RunTimestamp.json") -Force
Invoke-BenchmarkRun -Name "gemini_flash_caching_warm" -Method "caching"

Invoke-BenchmarkRun -Name "gemini_flash_masking_hard" -Method "masking"

Write-Summary "Gemini 2.5 Flash benchmark batch completed"
